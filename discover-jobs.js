...
async function fetchJson(url, label, timeoutMs = 15000) {
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: { "User-Agent": "AkashJobAI/1.0", Accept: "application/json,text/plain,*/*" }
    });
    if (!response.ok) {
...
async function fetchLever(slug, companyName) {
  const data = await fetchJson(`https://api.lever.co/v0/postings/${encodeURIComponent(slug)}?mode=json`, `lever:${slug}`, 30000);
...
async function fetchGreenhouse(slug, companyName) {
  const data = await fetchJson(`https://boards-api.greenhouse.io/v1/boards/${encodeURIComponent(slug)}/jobs?content=true`, `greenhouse:${slug}`, 30000);
...
async function fetchAshby(slug, companyName) {
  const data = await fetchJson(`https://api.ashbyhq.com/posting-api/job-board/${encodeURIComponent(slug)}`, `ashby:${slug}`, 30000);
  if (!data || !Array.isArray(data.jobs)) return [];
  return data.jobs.map(job => ({
    title: job.title || "Unknown role",
    company: companyName || slug,
...
async function fetchRemotive(query) {
  const data = await fetchJson(`https://remotive.com/api/remote-jobs?search=${encodeURIComponent(query)}`, `remotive:${query}`, 20000);
  if (!data || !Array.isArray(data.jobs)) return [];
  return data.jobs.map(job => ({
    title: job.title || "Unknown role",
...
async function fetchHimalayas(query) {
  const params = new URLSearchParams({ q: query, country: "India", seniority: "Entry-level", employment_type: "Full Time", sort: "recent", page: "1" });
  const data = await fetchJson(`https://himalayas.app/jobs/api/search?${params.toString()}`, `himalayas:${query}`, 20000);
  if (!data || !Array.isArray(data.jobs)) return [];
  return data.jobs.map(job => {
    const restrictions = Array.isArray(job.locationRestrictions) ? job.locationRestrictions : [];
    const location = restrictions.length ? `Remote - ${restrictions.join(", ")}` : "Remote - Worldwide";
    return {
      title: job.title || "Unknown role",
...
function isUsefulLocation(location, rules) {
  ...
  if (/hyderabad/i.test(value) || /bangalore|bengaluru/i.test(value)) return true;
...
function isObviouslySenior(candidate) {
  ...
  if (/\b(?:senior|sr\.?|staff|principal|lead|director|manager|head|vp|vice president)\b/i.test(title)) return true;
  if (/\b(?:engineer|developer|sde|swe)\s*(?:[- ]?(?:ii|iii|iv|v|vi)|[- ]?\d{2,})\b/i.test(title)) return true;
...
function hasEntryLevelSignal(candidate) {
  const text = normalize(`${candidate.title || ""} ${candidate.description || ""} ${(candidate.source_context || []).join(" ")}`);
...
function toQueueEntry(candidate, rules) {
  ...
  if (!hasEntryLevelSignal(candidate)) return null;
  return {
    title: String(candidate.title).slice(0, 180),
    company: candidate.company || "Unknown",
    location: candidate.location || "Unknown",
    description: String(candidate.description || "").slice(0, 30000),
    posting_url: href,
...
function dedupeAndMerge(existingQueue, newEntries) {
  ...
  for (const job of newEntries) {
    const index = merged.findIndex(existing =>
      (normalize(existing.posting_url) && normalize(existing.posting_url) === normalize(job.posting_url)) ||
      (normalize(existing.title) === normalize(job.title) && normalize(existing.company) === normalize(job.company) && normalize(existing.location) === normalize(job.location))
    );
    if (index === -1) {
      merged.push(job);
      added += 1;
      ...
      merged[index] = {
        ...old,
        ...job,
        status: old.application_status === "applied" ? old.status : "needs_verification",
...
async function collectCandidates(rules) {
  ...
  for (const target of companies) {
    ...
    let jobs = [];
    if (target.ats === "lever") jobs = await fetchLever(target.slug, target.name);
    else if (target.ats === "greenhouse") jobs = await fetchGreenhouse(target.slug, target.name);
    else if (target.ats === "ashby") jobs = await fetchAshby(target.slug, target.name);
    else { console.warn(`⚠️ Unknown ATS: ${target.ats} (${target.name || target.slug})`); continue; }
    console.log(`  ${target.name || target.slug}: ${jobs.length} postings`);
    candidates.push(...jobs);
    await sleep(200);
  }
  for (const query of ["software engineer", "software developer", "backend engineer", "full stack developer"]) {
...
async function main() {
  ...
  console.log(`✅ Matched fresh/entry-level candidates: ${discovered.length}`);
...
module.exports = { normalize, canonicalUrl, looksLikeTargetTitle, fetchLever, fetchGreenhouse, fetchAshby, fetchRemotive, fetchHimalayas, toQueueEntry, dedupeAndMerge, collectCandidates, hasEntryLevelSignal };

if (require.main === module) main().catch(error => { console.error("\n❌ DISCOVERY ERROR:", error.message); process.exit(1); });
